...
class PCIE_test:
    ...
    def test_content(self):
        ...
        with open(self.logname, 'a+') as f:
            f.write("PCIE Ethernet info is:\r  '%s'\rPCIE chip info is:\r  '%s'\rPCIE external graphic card is:\r  '%s'"
                    "\rPCIE Ethernet Value is:\r  '%s'\rPCIE chip value is:\r"
                    "  '%s'\rPCIE 1st external card value is:\r  '%s'\rPCIE 2nd external card value is:\r  '%s'\r"
                    % (pcie_Ethernet_info.decode(), pcie_chip_info.decode(), pcie_external_graphic_card.decode(),
                       pcie_Ethernet_value.decode(), pcie_chip_value.decode(), pcie_1st_external_card_value.decode(),
                       pcie_2nd_external_card_value.decode()))
